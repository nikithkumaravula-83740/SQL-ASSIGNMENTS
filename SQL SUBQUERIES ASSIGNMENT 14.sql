-- NAME- Maneesh Singh Samant
-- COURSE - Data Analytics with GEN AI
-- ASSIGNMENT - SubQueries Assignment
show databases;
...
SELECT name 
FROM Employee_dataset 
WHERE salary > (SELECT salary FROM Employee_dataset 
WHERE name='Shubham');
...
FROM Employee_dataset
WHERE department_id = (
SELECT department_id FROM employee_dataset 
WHERE name='Abhishek');
...
SELECT department_id 
FROM Employee_dataset 
WHERE emp_id = (SELECT emp_id FROM Sales ORDER BY sale_amount 
DESC LIMIT 1));
...
FROM Sales 
WHERE emp_id IN (
SELECT emp_id FROM Employee_dataset WHERE salary > (SELECT AVG(salary) FROM Employee_dataset)
);
